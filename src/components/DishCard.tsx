...
export const DishCard: React.FC<DishCardProps> = ({ dish, onOrderClick }) => {
  ...
  const handleOrder = (e: React.MouseEvent) => {
    ...
    if (onOrderClick) {
      ...
      // Navigate to Menu page with category pre-selected
      navigate(`/menu?category=${encodeURIComponent(dish.category)}`);
    }
  };
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -8 }}
      className="group relative bg-[#F6EFE3] rounded-2xl overflow-hidden border border-brand-dark/10 shadow-sm transition-shadow hover:shadow-xl flex flex-col h-full"
    >
      {/* Popular / Veg badges */}
